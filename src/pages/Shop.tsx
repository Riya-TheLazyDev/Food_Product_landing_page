...
export default function Shop() {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      else if (filters.price === "50-100") matchesPrice = product.price >= 50 && product.price <= 100;
  ...
  const getGroupedProducts = (category: string) => {
    if (category === "Our Favourites Right Now") {
      // Only show favourites if no filters are active (or just filter favourites from the filtered set)
